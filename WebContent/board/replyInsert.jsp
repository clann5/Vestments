...
<script>
	...
	function fn_replyInsert(f){
		f.action = '/Home/replyInsert.board';
		f.submit();
	}
	...
	<form method="post">
		...
		<input type="hidden" name="page" value="${page}" >			<!-- boardView.jsp에서 hidden 으로 넘긴 값 -->
		<input type="hidden" name="bNo" value="${param.bNo}" />		<!-- boardView.jsp에서 hidden 으로 넘긴 값 -->
		<input type="button" value="댓글 등록하기" onclick="fn_replyInsert(this.form)"/>
